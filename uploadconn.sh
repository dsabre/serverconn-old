...
if [ -z "$numServer" ]; then
	...
	while read line; do
		...
		if $(echo $line | grep --quiet '\[HEAD\]'); then
			...
			# get the username form the line
			username=$(whoami)
			if [[ $str == *"@"* ]]; then
				username=$(echo $str |awk -F '@' {'print $1'})
			fi
			
			echo -e "\t$counter - $str:/home/$username"
...
# get the username form the selected line
str=$(echo $line | sed -r 's/\[NOPASS\]//g' | sed -r 's/\[PASSGPG=.*\]//g' | sed -r 's/\[PASS=.*\]//g')
username=$(whoami)
if [[ $str == *"@"* ]]; then
	username=$(echo $str |awk -F '@' {'print $1'})
fi
str="$str:/home/$username"
